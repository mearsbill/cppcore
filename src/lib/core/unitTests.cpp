...
abcResult_e testHashList()
{
	...
	tList->abcHashList_c::init(6,4,400);
	tList->addSliceTail(6,t1);		// should fail... disabled method
	if (result != ABC_FAIL)
	{
		FATAL_ERROR_G(ABC_REASON_TEST_FAILED);
		return(ABC_FAIL);
	...
	DEBUG_A("Single Node added to list\n");
